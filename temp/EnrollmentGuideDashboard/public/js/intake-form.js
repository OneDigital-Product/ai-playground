...
function initializeSectionToggles() {
    const sectionRadios = document.querySelectorAll('input[type="radio"][name*="_changed"]');
...
function showAutoSaveIndicator() {
    ...
    if (!indicator) {
        ...
        indicator.className = 'fixed bottom-4 right-4 bg-green-500 text-white px-3 py-1 rounded text-sm opacity-0 transition-opacity';
...
document.addEventListener('keydown', function(e) {
    ...
    if (e.altKey && e.key === 'ArrowLeft') {
        e.preventDefault();
        const prevButton = document.querySelector('button[onclick*="history.back"]');
        if (prevButton) {
            prevButton.click();
        }
    }
